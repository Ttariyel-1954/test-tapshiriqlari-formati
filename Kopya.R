                ...
                box(title = "🎮 İnteraktiv Test Sessiyası", status = "success", 
                    ...
                    fluidRow(
                      ...
                      column(4,
                             h4("⏰ Vaxt Sayğacı"),
                             div(class = "timer",
                                 div(style = "margin-bottom: 5px; font-size: 14px; color: #856404;", "Qalan Vaxt:"),
                                 h1(textOutput("timer_text"), style = "margin: 0; font-size: 2.5em;"))
                      )
                    ),
...
server <- function(input, output, session) {
  ...
  # Reactive dəyişənlər - timer_text silindi
  ...
  # Test başlatma - display text də set et
  observeEvent(input$start_test, {
    ...
    # İlk display text
    minutes <- floor(values$remaining_time / 60)
    seconds <- values$remaining_time %% 60
    timer_values$display_text <- sprintf("%02d:%02d", minutes, seconds)
    timer_values$last_update <- Sys.time()
    
    # Debug üçün
    cat("Timer başladı: ", values$current_question$time_limit, " saniyə (Real time: ", format(Sys.time(), "%H:%M:%S"), ")\n")
    
    showNotification("🚀 Test başladı! Uğurlar!", type = "default", duration = 3)
  })
  
  # Timer - tamamilə sadə yanaşma
  timer_values <- reactiveValues(
    display_text = "--:--",
    last_update = Sys.time()
  )
  
  # Timer observe - yalnız vaxtı azalt
  observe({
    invalidateLater(1000, session)
    
    # Zaman fərqini yoxla (double execution qarşısı almaq üçün)
    current_time <- Sys.time()
    time_diff <- as.numeric(difftime(current_time, timer_values$last_update, units = "secs"))
    
    if(isTRUE(values$timer_active) && time_diff >= 0.9) {  # 0.9 saniyə threshold
      timer_values$last_update <- current_time
      
      if(values$remaining_time > 0) {
        ...
        if(values$remaining_time <= 30 && values$remaining_time > 0) {
          timer_values$display_text <- paste("⚠️", time_text)
        } else {
          timer_values$display_text <- time_text
        }
        
        # Debug - real time check
        if(values$remaining_time %% 30 == 0) {
          cat("Qalan vaxt: ", values$remaining_time, " saniyə (Real time: ", format(current_time, "%H:%M:%S"), ")\n")
        }
      } else {
        values$timer_active <- FALSE
        timer_values$display_text <- "00:00"
        cat("Vaxt bitdi! (Real time: ", format(current_time, "%H:%M:%S"), ")\n")
        showNotification("⏰ Vaxt bitdi!", type = "warning", duration = 5)
      }
    }
  })
  
  # Timer göstəricisi - sadəcə display text göstər
  output$timer_text <- renderText({
    timer_values$display_text
  })
  
  # Test məlumatları - timer update-dən ayrı 
  output$test_info <- renderText({
    # Yalnız test başladıqda dəyişən məlumatlar
    current_question <- values$current_question
    test_started <- values$test_started
    
    if(!is.null(current_question)) {
      paste(
        "📋 Format:", current_question$title,
        "\n📚 Mövzu:", current_question$topic,
        "\n⭐ Çətinlik:", current_question$difficulty_level, "/5",
        "\n🎯 Sual ID:", current_question$question_id,
        "\n⏱️ Vaxt limiti:", current_question$time_limit, "saniyə"
      )
    } else {
  ...
  # Ayrıca timer status göstəricisi
  output$timer_status <- renderText({
    timer_active <- values$timer_active
    test_started <- values$test_started
    
    if(isTRUE(timer_active)) {
      "🟢 Timer aktiv"
    } else if(isTRUE(test_started)) {
      "🔴 Vaxt bitdi"
    } else {
      "⚪ Test başlamayıb"
    }
